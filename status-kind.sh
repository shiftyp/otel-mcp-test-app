...
# Script to check status of services in Kind cluster
# Usage: ./kind-status.sh [--watch]
...
print_header() {
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}                    Kind E-Commerce Platform Status                     ${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
}
...
check_cluster() {
    if ! kubectl cluster-info &>/dev/null; then
        ...
        echo -e "${YELLOW}Make sure Kind cluster is running and kubectl is configured${NC}"
        exit 1
    else
        echo -e "${GREEN}✓ Connected to Kubernetes cluster${NC}"
        
        # Show Kind cluster info if available
        if command -v kind &> /dev/null; then
            local clusters=$(kind get clusters 2>/dev/null | head -n 1)
            if [ -n "$clusters" ]; then
                echo -e "${GREEN}✓ Kind cluster: $clusters${NC}"
            fi
        fi
    fi
}
...
check_registry() {
    ...
    if docker ps | grep -q "ecommerce-registry"; then
        echo -e "${GREEN}✓ Registry container is running${NC}"
        
        # Check registry health
        if curl -s -f http://localhost:5000/v2/_catalog > /dev/null 2>&1; then
            echo -e "${GREEN}✓ Registry is healthy at localhost:5000${NC}"
            
            # Show catalog
            local catalog=$(curl -s http://localhost:5000/v2/_catalog | jq -r '.repositories[]' 2>/dev/null | wc -l)
            echo -e "  Images in registry: $catalog"
        else
            echo -e "${RED}✗ Registry is not responding${NC}"
        fi
    else
        echo -e "${YELLOW}⚠ Registry container is not running${NC}"
        echo -e "  Run ./setup-kind.sh to set up the registry"
    fi
}
...
show_logs_commands() {
    ...
    echo "  docker logs ecommerce-registry"
}
