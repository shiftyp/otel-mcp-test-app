...
# Script to build and deploy services to Kind cluster
# Usage: ./deploy-service-kind.sh [service1] [service2] ...
...
check_cluster() {
    if ! kubectl cluster-info &>/dev/null; then
        ...
        print_info "Ensure Kind cluster is running and kubectl is configured"
        exit 1
    fi
}
...
check_registry() {
    if [ "$(docker inspect -f '{{.State.Running}}' "${REGISTRY_NAME}" 2>/dev/null || true)" != 'true' ]; then
        print_error "Registry '${REGISTRY_NAME}' is not running"
        print_info "Start it with: ./setup-kind-registry.sh"
        exit 1
    fi
    
    # Check registry health
    if ! curl -s -f http://${REGISTRY_URL}/v2/_catalog > /dev/null 2>&1; then
        print_error "Registry is not healthy at ${REGISTRY_URL}"
        exit 1
    fi
}
...
build_and_push_service() {
    ...
    # Special handling for nginx - just use the base image
    if [ "$service_name" == "nginx" ]; then
        # Tag the official nginx image
        docker pull nginx:alpine
        docker tag nginx:alpine "$image_name"
        print_success "Tagged nginx:alpine as $image_name"
    else
        # Build image
    ...
    # Update image reference to use new registry
    local k8s_registry_image="${REGISTRY_URL}/$image_name"
    
    # Determine the deployment name and container name
    local deployment_name=$service_name
    local container_name=$service_name
    
    # Special handling for nginx deployment name
    if [ "$service_name" == "nginx" ]; then
        deployment_name="frontend-nginx"
    fi
    
    # Update the deployment with the new image
    kubectl -n $NAMESPACE set image deployment/$deployment_name $container_name=$k8s_registry_image
...
# Main script
main() {
    print_info "Kind Service Deployment Tool"
    echo ""
